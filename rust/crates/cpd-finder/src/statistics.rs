...
pub fn compute(sources: &[SourceFile], clones: &[CpdClone]) -> Statistics {
    let total_lines: u64 = sources.iter()
    ...
    let duplicated_lines: u64 = clones.iter().map(|c| {
        c.fragment_a.end.line.saturating_sub(c.fragment_a.start.line) as u64 + 1
    }).sum();
    ...
    for source in sources {
        let entry = formats.entry(source.format.clone()).or_insert_with(|| StatRow {
        ...
        entry.lines += source.tokens.iter().map(|t| t.start.line).max().unwrap_or(0) as u64;
    }
    for clone in clones {
        if let Some(entry) = formats.get_mut(&clone.format) {
            entry.clones += 1;
            entry.duplicated_lines += clone.fragment_a.end.line
                .saturating_sub(clone.fragment_a.start.line) as u64 + 1;
...
mod tests {
    ...
    fn loc(line: u32) -> Location {
        Location { line, column: 0, offset: 0 }
    }
