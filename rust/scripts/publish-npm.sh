...
#
# Modes:
#   --all            Build and publish all 8 platform packages, then publish main
#   --target <key>   Build and publish a single platform package
#   (default)        Build and publish for the current platform only
#
# Targets: linux-x64-gnu, linux-arm64-gnu, linux-x64-musl, linux-arm64-musl,
...
#   The main cpd package is only published if all 8 are on npm.
#
# Environment:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo ""
      echo "Options:"
      echo "  --all              Build and publish all 8 platform packages"
      ...
      echo ""
      echo "Available targets: linux-x64-gnu, linux-arm64-gnu, linux-x64-musl, linux-arm64-musl,"
      ...
      echo "  By default the main cpd package requires all 8 platforms on npm."
...
ALL_TARGET_KEYS="linux-x64-gnu linux-arm64-gnu linux-x64-musl linux-arm64-musl darwin-arm64 darwin-x64 windows-x64-msvc windows-arm64-msvc"
