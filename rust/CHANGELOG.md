...
- **basta detects the project's frameworks, and the list is data.** What a framework starts without an import — a router's pages, a runtime's plugin directories, handlers picked up by convention — used to be three hard-coded cases (Nuxt, Nitro, WXT). It is now a table of some fifty frameworks compiled into the binary, [`frameworks.yaml`](crates/basta/frameworks.yaml): Next.js, Nuxt, Nitro, WXT, Plasmo, Remix, React Router, SvelteKit, Astro, SolidStart, TanStack Start, Qwik City, Gatsby, RedwoodJS, Angular, NestJS, AdonisJS, Sails, `@fastify/autoload`, Strapi, Medusa, Ember, Quasar, React Native, Expo, Cloudflare Workers, Vercel, Netlify, Serverless, Trigger.dev, Docusaurus, VitePress, Eleventy, Storybook, Histoire, Jest, Vitest, Mocha, AVA, Playwright, Cypress, Cucumber, Prisma, Knex, Sequelize, TypeORM, Hardhat and Create React App. Frameworks are not alternatives: every one that matches is in force at once and roots its own files (a Next application with Storybook, Vitest and Prisma is four of them). Each is detected per directory — so per package in a monorepo — by any one of: its config file by name, the package among the dependencies of `package.json`, or its section in `package.json`; its entry globs and whole directories are anchored at the directory it was found in, and its config is read for the literals that move them (`srcDir`, `entrypointsDir`, `appDirectory`, `sourceRoot` in `nest-cli.json`, `imports: false`). The console report names what was found (`Frameworks: next (apps/web), vitest`), in `basta` and `jscpd --dead-code` alike. New `basta` flags: `--list-frameworks`, `--framework <name>` (take one as present, for a scan that starts below its `package.json`), `--no-frameworks`, and `--frameworks-config <file>` — YAML or JSON in the table's own shape, also picked up as `basta.frameworks.{yaml,yml,json}` in the working directory — which adds a framework basta has never heard of or replaces a built-in by name. A definitions file that does not load is an error, not a warning: going on without it would report as dead exactly the files it exists to keep alive. A definition also lists the **names its framework reads** out of the project's code, under `globals` — `getServerSideProps`, `generateMetadata` and the route segment config in Next, `loader`/`action`/`meta` in Remix and React Router, `load`/`actions`/`prerender` and the hooks in SvelteKit, `getStaticPaths` in Astro, the Gatsby Node/browser/SSR APIs, Angular's and Nest's lifecycle methods, `onRequest*` in Cloudflare Pages Functions, `handler` in Netlify and Serverless functions, and more. A declaration under one of them is used by the framework: it is never reported — not under `--include-entry-exports`, not as an unused member — and whatever it calls stays reachable. A bare name holds in every file of the project; `{ names, files }` ties names to the files they are read from, so `loader` is Remix's word under the app directory and anybody's word everywhere else (which also covers a React Router route named from `routes.ts`, a file reached by a string whose `loader` used to read as an export nothing imports). Nitro additionally follows `srcDir` and roots `tasks/`. See [`fixtures/dead-code-demo`](../fixtures/dead-code-demo/README.md#frameworks).

- **Dead-code settings get a section of their own in `.jscpd.json`.** The `deadCode` key (also `dead-code`, `basta`) was a boolean that switched the mode on; it is now either that or an object: `enabled`, `categories`, `minConfidence`, `minLines`, `entry`, `ignore`, `includeTests`, `includeEntryExports`, `threshold`, and the framework settings — `frameworks` (definitions inline, in the shape of `frameworks.yaml`), `frameworksConfig`, `framework`, `noFrameworks`. The section configures without switching the mode on unless it says `"enabled": true`, so clone and dead-code settings sit side by side and the command line picks the run; `"deadCode": true` still works as the switch. A flag beats the section, and the section beats the flat top-level keys (`minConfidence`, `entry`, `deadCodeCategories`, …), which keep working. Two keys exist only here because the top-level ones mean something else: `minLines` (the top-level one is about clones and is never carried into a dead-code run) and `threshold` (a dead-code budget separate from the duplication one; without it the run falls back to the top-level value as before). A misspelled key inside the section is reported by name. `jscpd --dead-code` and `--dashboard` now also pick up a `basta.frameworks.{yaml,yml,json}` from the working directory. The standalone `basta` binary reads the same section from the same file — new `-c, --config <file>`, otherwise the `.jscpd.json`, `.config/jscpd.json` or `package.json` of the working directory — so both tools started in one directory agree. See [`fixtures/dead-code-demo`](../fixtures/dead-code-demo/README.md#in-the-config-file).

### Fixes

Found by running basta beside knip and fallow on 55 repositories — the GitHub trending lists for JavaScript, TypeScript, Vue, Svelte and Astro, and the sources of Nuxt, Next.js, Svelte, Gatsby and Astro:

- **`--dead-code` was quadratic in the project's path aliases.** Every import was tested against every alias of the project, and the expensive question came first: is the importer under the config that declared this alias, compared component by component through a prefix every file shares. A monorepo whose 150 packages each list a few hundred `paths` took 198 s (ever-gauzy) and Next.js 83 s. The pattern is now matched against the specifier first, which fails on the first byte for nearly every alias: 3.3 s and 4.4 s, findings byte-identical.
- **A catch-all alias made every package import expensive.** `"*": ["./*", "../../node_modules/*"]` sends `react` through the resolver from every file, and each miss built and hashed some twenty candidate paths. The module index now answers exactly, in two lookups, whether a base could name any module under any extension or as a directory index (LibreChat: 4.0 s → 0.86 s, findings identical).
- **Vite's `'@': '/src'` was read as the root of the file system.** A leading `/` in an alias target means the project root, the way it does in a URL, and is one of the most common spellings; every `@/` import of such a project was lost and its components reported as unused files at 95% (MoeKoeMusic: 55 → 0).
- **``require(`./x`)`` was not read.** A template literal with nothing interpolated is as static as a quoted string, and Gatsby writes every string that way (584 → 352 unused files there).
- **An arrow function in a Svelte or Astro attribute ended the tag.** In `<script on:load={() => { … }} src=…>` the `>` of `=>` was taken for the end of the tag, the rest of the attribute was handed to the JavaScript parser as the component's script, the file failed to parse and everything it imports was reported (cobalt: 37 → 8 unused files).
- **Django's migrations and management commands were reported as unused files**, along with `admin.py`, `apps.py`, template tags and the modules settings name by dotted path. Django, Alembic and Scrapy are now rows of the framework table, detected by `manage.py`, `alembic.ini` and `scrapy.cfg` (AdventureLog: 144 → 6 Python files).
- **A file named only by an extension-less path string is now doubted rather than declared dead.** `resolve(distDir, 'runtime/handlers/island')` is how a framework registers a file it loads itself; it is not an import and creates no edge, but a file whose own path ends the same way carries a new reason, `path-appears-in-string` ("its path appears in a string literal"), worth 40 points — under the default floor of 60, still listed at `--min-confidence 0` (Nuxt: 83 → 46). The reason is an added value of `reasons` in the JSON report.
