...
export class Detector {
  ...
  public detect(source: ISource): IClone[] {
    ...
    if (sourceExist && sourceExist.last_update === source.last_update) {
    ...
    const tokens: IToken[] = this.tokenize(source.source, source.format)
      .map(t => Object.assign(t, {sourceId}));

    const tokenMaps: TokensMap[] = this.generateMapsForFormats(tokens);

    let clones: IClone[] = [];

    tokenMaps.forEach(tokenMap => clones = clones.concat(...this.detectByMap(tokenMap)));

    return clones.map(clone => Object.assign(clone, {is_new: true}));
  }
  ...
  private detectByMap(tokenMap: TokensMap): IClone[] {
    ...
    if (tokenMap.getLength() >= this.options.minTokens) {
      ...
      const HashesStore: IStore<IMapFrame> = StoresManager.get('hashes.' + tokenMap.getFormat());
      ...
      for (const mapFrame of tokenMap) {
        if (HashesStore.has(mapFrame.id)) {
          ...
          if (isClone && start && end) {
            const clone: IClone = Detector.createClone(start, end, tokenMap.getFormat());
  ...
  private static createClone(startMap: IMapFrame, endMap: IMapFrame, format: string): IClone {
    const hashesStore: IStore<IMapFrame> = StoresManager.get('hashes.' + format);

    const sourceStart: IMapFrame = hashesStore.get(startMap.id);
    const sourceEnd: IMapFrame = hashesStore.get(endMap.id);

    const fragment: string = Detector.getFragment(startMap.start.sourceId, startMap.start.range[0], endMap.end.range[1]);
  ...
  private static getFragment(sourceId: string, start: number, end: number): string {
    const sourcesStore: IStore<ISource> = StoresManager.get('source');
  ...
  private getModeHandler(): (token: IToken) => boolean {
    if (!this.options.hasOwnProperty('mode') || !this.options.mode) {
      this.options.mode = 'mild';
    }
  ...
  private isCloneLinesBiggerLimit(clone: IClone): boolean {
    return (clone.duplicationA.end.loc.end.line - clone.duplicationA.start.loc.end.line >= this.options.minLines);
  }
}
