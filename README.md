...
- **Type-4 semantic clones (experimental)** — `--semantic` embeds every function of JavaScript, TypeScript, Vue, Svelte, Astro, Python, Rust, Go, Java, Kotlin, C#, C, C++, PHP, Ruby, Scala and Swift code with a code embedding model that jscpd runs itself (`jscpd --semantic-download` once), or with any OpenAI-compatible API, and reports functions that do the same thing written differently: the same feature implemented twice in one language, or a rule your Rust backend enforces and your Svelte frontend repeats (see [docs](docs/rust.md#semantic-clones-with---semantic-experimental) and [the demo](fixtures/semantic-demo/README.md))
